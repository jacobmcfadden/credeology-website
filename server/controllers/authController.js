...
module.exports = {
        ...
        if(!user[0]) {
            ...
            if(authenticated) {
                req.session.user = {
                    ...
                    isAuthenticated: user[0].two_factor_auth === true ? true : false,
                    isLoggedIn: true
                }
    ...
    recoverAccount: async (req, res) => {
        ...
        const user = await db.auth.recover_user_email([email]);
        if(user[0]) {
            return res.status(200).send(email)
        } else {
            res.status(401).send('You have entered invalid account credentials.')
        }
    ...
    resetPassword: async (req, res) => {
        ...
        let year = date_ob.getFullYear();
        const todayDate = (year + "-" + month + "-" + date);
        console.log(existingUser[0].id, hash, todayDate)
    ...
    sendPhoneCode: async (req, res) => {
        ...
        const user = await db.auth.find_user([phone]);
                ...
                if(authenticated) {
                    ...
                    if(!codeVerified) {
                        ...
                        req.session.user = {
                            ...
                            isEmailVerified: user[0].verify_email ? true : false,
                            isPhoneVerified: user[0].verify_phone ? true : false,
                            twoFactorAuth: user[0].two_factor_auth
                                        }
                        return res.status(200).send(req.session.user)
                    }
                } else {
    ...
    twoFactorAuthentication: async (req, res) => {
        const db = req.app.get('db');
        const {userInput} = req.body;
        const {phone} = req.session.user;
        const user = await db.auth.find_user([phone]);
